...
- **Locale-aware column delimiter (bug fix, 2026-07-22, superseding an earlier `sep=,` attempt the same day):** Excel picks its CSV column separator from the OS region's number format, not the file's actual delimiter — on Turkish (and many other non-US/UK) Windows locales the decimal comma makes Excel expect `;`, so a correctly comma-delimited file opened as one unsplit column per row. The first fix was an Excel `sep=,` directive line; it was reverted within the same session when it turned out to make Excel skip its normal BOM-aware open path, corrupting Turkish letters (ı/İ/ğ/Ğ/ş/Ş) instead. The delimiter itself is now locale-matched end to end: the Custom UI export surface resolves the viewer's `Locale` (already does, for i18n) and sends the matching separator through `ExportFilePayload.csvDelimiter` (`,` default, `;` for `tr`) to `domain/csv.ts`'s `toCsv`/`toCsvRow`, which quote fields relative to whichever delimiter is active. No directive line, no BOM-detection risk.
