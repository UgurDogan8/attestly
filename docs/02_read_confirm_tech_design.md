...
`asApp()` REST usage is thus limited to: (a) the **other-user cannot-view checks** (v1 API, not deprecated; scope `read:content.permission:confluence`) — run only for pages that already passed the viewer-visibility filter; and (b) the deleted-vs-restricted existence probe above. The check call, as live-validated in spike M0-2:
...
// 200 → { hasPermission: boolean }
// 404 → NotFoundException "No user with id …" for unknown/erased accountIds — map to deleted-user, never crash the row
``` Live-measured ~150–250 ms per permission check: per-user fan-out on drill-down must run concurrently (batches of ~10) and cache per (page, user) within an invocation; a full 100-user drill-down is ~2–3 s, not 20 s sequential. **Unknown/deleted accountId returns HTTP 404** (`NotFoundException`), not `hasPermission: false` — map 404 to the deleted-user rendering, never crash the row.
...
Entities and indexes are specified in the [data model spec](./03_read_confirm_data_model.md). Query shapes as live-validated in spike M0-1 (`@forge/kvs`; warm latencies at 1k records in §11.1):

```ts
import { kvs, Sort, WhereConditions } from '@forge/kvs';

// Macro hot path — latest confirmed version for (page, user): one read, ~50 ms p50
const latest = await kvs.entity('confirmation').query()
  .index('by-page-user', { partition: [pageId, accountId] })
  .sort(Sort.DESC).limit(1).getMany();          // results[0]?.value.pageVersion

// Drill-down / export — all records for a page, cursor-paged (~100 ms per 100-row page)
let cursor: string | undefined;
do {
  let q = kvs.entity('confirmation').query()
    .index('by-page', { partition: [pageId] }).limit(100);
  if (cursor) q = q.cursor(cursor);
  const r = await q.getMany();                  // process r.results…
  cursor = r.nextCursor;
} while (cursor);

// Dashboard list — tracked pages site-wide; add .where for a space filter
const tracked = await kvs.entity('page-config').query()
  .index('tracked', { partition: [true] })
  .where(WhereConditions.equalTo(spaceKey))     // omit for all spaces
  .limit(100).getMany();
```

Key platform constraints the design must respect (verified against Forge docs, Jul 2026):
...
1. ~~Confirm KVS custom entity index shapes; measure query latency at 1k records~~ — **resolved** (spike app deployed + measured on Forge, Jul 2026; app deleted after M0 — full spike code in git history @ `f716064`). Shapes validated with one addition: a third `confirmation` index `by-page-user` (partition `pageId, accountId`, range `pageVersion`) is required for the macro hot path — see data model §2.1 (manifest YAML) and §5 (query code). Warm latencies at 1k records (10 iterations): macro latest-version lookup 49ms p50 / 52ms p95; 100-row query page ~100ms p50 / ~150ms p95 (drill-down first paint, user history, tracked list all alike); full 1k-record drain via 10 sequential cursor pages 1.02s p50 / 1.09s p95; cold start adds ~150–200ms to the first invocation → extrapolated ~10s for a 10k-record export, confirming the chunked-export design (§4) is necessary and sufficient. Platform findings: entity names must be lowercase (`page-config`, `config-audit`); index `range` holds exactly one attribute; `filters` on non-indexed attributes exist but are in-memory (may return empty pages — avoid for correctness-critical paths).
2. ~~`asUser` vs `asApp` split; validate the content permission check API~~ — **resolved** (spike M0-2, live-tested Jul 2026 via a temporary spike webtrigger; code in git history @ `f716064`): three-tier model now normative in §4, including the validated check call. Key facts: bridge `requestConfluence` = current user (display-only use); `asUser()` = server-authoritative confirm path, but unavailable outside UI-invoked resolvers (reminders must be `asApp`); permission-check API validated `asApp` against a real site — `hasPermission` returned for real users, **404 for unknown accountIds**, ~150–250 ms/check → concurrent batching required for drill-down fan-out. Residual (verify during Epic B implementation): a real `hasPermission: false` case with a genuinely restricted page — the test site page had no restrictions. **Amended after review:** dashboard page resolution moved from `asApp` to bulk `asUser` so the dashboard/exports can never reveal pages the viewing manager can't access (visibility rule in §4) — `asApp` is confined to other-user permission checks and the deleted-vs-restricted probe.
...
7. ~~Resource `entry` property~~ — **resolved: adopted** (spike M0-7, live-validated Jul 2026 via spike app v3.1.0; app deleted after M0, code in git history @ `f716064`). Still Preview, but Atlassian's `forge module add` scaffolds multi-entry by default; macro + globalPage + globalSettings deployed successfully against a multi-entry resource on Confluence (contentBylineItem lint-validated only — a dev byline would render on every page of the test site). §2/§3 rewritten around one `static/app` Vite MPA build. Gotchas captured in §3 (root-level full-HTML entries, `globalPage` needs `route`) and §10 (CLI ≥ 13 + Node 22/24). Side-findings: module additions confirmed minor-version in practice (3.0.0 → 3.1.0, no re-consent — corroborates M0-5); `webtrigger` modules break Runs on Atlassian eligibility (§7). Residual for M1: visual render check of each surface once real UIs exist; re-check `entry` GA status at marketplace submission.
