...
| **No write scopes** | `permissions.scopes` in `manifest.yml` is six read/storage scopes (below) and zero `write:*` scopes. The app cannot create, edit, or delete any Confluence content, comment, or permission. |
...
## Scopes requested (six, all read/storage)
...
| `read:user:confluence` | Display-name resolution at render/export time |
...
| `read:content-details:confluence` | The Confluence-admin check (`GET /wiki/rest/api/user/current?expand=operations`) used to gate the settings page — this is the scope Atlassian's own REST reference lists for that endpoint, not `read:user:confluence` |
