...
(() => {
  ...
  var require_invoke = __commonJS({
    "node_modules/@forge/bridge/out/invoke/invoke.js"(exports) {
      ...
      var isMetadataWrappedResponse = (response) => {
      ...
      var _invoke = async (functionKey, payload, metadata) => {
        ...
        if (metadata) {
          if (!isMetadataWrappedResponse(response)) {
            return { body: response, metadata: void 0 };
          }
...
`);
      (function(undefined2) {
        ...
        function iFrameListener(event) {
          ...
          function findTarget(location) {
            ...
            if (target) {
              ...
              log2(iframeId, "In page link #" + hash + " not found");
            } else {
              jumpToParent();
            }
          }
        ...
        function resizeIFrames(event) {
          ...
          log2("window", "Trigger event: " + event);
          debouce(resize, 16);
        }
        function tabVisible() {
          function resize() {
  ...
  var require_iframeResizer_contentWindow = __commonJS({
    "node_modules/iframe-resizer/js/iframeResizer.contentWindow.js"(exports, module) {
      (function(undefined2) {
        ...
        function init() {
          readDataFromParent();
          log2("Initialising iFrame (" + window.location.href + ")");
          readDataFromPage();
          setMargin();
        ...
        function readDataFromPage() {
          function readData() {
            ...
            log2("Reading data from page: " + JSON.stringify(data));
          ...
          function setupCustomCalcMethods(calcMode, calcFunc) {
            if ("function" === typeof calcMode) {
              log2("Setup custom " + calcFunc + "CalcMethod");
          ...
          log2("TargetOrigin for parent set to: " + targetOriginDefault);
        }
        ...
        function stopInfiniteResizingOfIFrame() {
          ...
          log2('HTML & body height set to "auto"');
        }
        ...
        function checkCalcMode(calcMode, calcModeDefault, modes, type) {
          if (calcModeDefault !== calcMode) {
            ...
            log2(type + ' calculation method set to "' + calcMode + '"');
          }
          return calcMode;
        }
        ...
        function setupInPageLinks() {
          ...
          function enableInPageLinks() {
            if (Array.prototype.forEach && document.querySelectorAll) {
              log2("Setting up location.hash handlers");
              bindAnchors();
              bindLocationHash();
              initCheck();
            } else {
          ...
          if (inPageLinks.enable) {
            ...
            log2("In page linking not enabled");
          }
          return {
            findTarget
          };
        ...
        function setupPublicMethods() {
          ...
          win.parentIFrame = {
            ...
            setTargetOrigin: function setTargetOriginF(targetOrigin) {
              log2("Set targetOrigin: " + targetOrigin);
        ...
        function setupBodyMutationObserver() {
          function addImageLoadListners(mutation) {
            function addImageLoadListener(element) {
              if (false === element.complete) {
                log2("Attach listeners to " + element.src);
          ...
          function removeImageLoadListener(element) {
            log2("Remove listeners from " + element.src);
          ...
          function createMutationObserver() {
            ...
            observer = new MutationObserver(mutationObserved);
            log2("Create body MutationObserver");
          ...
          return {
            disconnect: function() {
              if ("disconnect" in observer) {
                log2("Disconnect body MutationObserver");
                observer.disconnect();
        ...
        function setupMutationObserver() {
          ...
          if (window.MutationObserver || window.WebKitMutationObserver) {
            ...
            log2("MutationObserver not supported in this browser!");
            initInterval();
          }
        }
        ...
        function chkEventThottle(timer) {
          if (timer > throttledTimer / 2) {
            ...
            log2("Event throttle increased to " + throttledTimer + "ms");
          }
        }
        function getMaxElement(side, elements) {
          ...
          log2("Parsed " + elementsLength + " HTML elements");
          log2("Element position calculated in " + timer + "ms");
        ...
        function sizeIFrame(triggerEvent, triggerEventDesc, customHeight, customWidth) {
          ...
          function logIgnored() {
            log2("No change in size detected");
          }
        ...
        function sendSize(triggerEvent, triggerEventDesc, customHeight, customWidth) {
          function recordTrigger() {
            if (!(triggerEvent in { reset: 1, resetPage: 1, init: 1 })) {
              log2("Trigger event: " + triggerEventDesc);
            }
          }
          ...
          if (isDoubleFiredEvent()) {
            log2("Trigger event cancelled: " + triggerEvent);
          } else {
            recordTrigger();
        ...
        function lockTrigger() {
          if (!triggerLocked) {
            triggerLocked = true;
            log2("Trigger event lock on");
          }
          clearTimeout(triggerLockedTimer);
          triggerLockedTimer = setTimeout(function() {
            triggerLocked = false;
            log2("Trigger event lock off");
            log2("--");
          }, eventCancelTimer);
        }
        ...
        function resetIFrame(triggerEventDesc) {
          ...
          log2("Reset trigger event: " + triggerEventDesc);
          lockTrigger();
          triggerReset("reset");
          heightCalcMode = hcm;
        }
        function sendMsg(height2, width2, triggerEvent, msg, targetOrigin) {
          function setTargetOrigin() {
            if (undefined2 === targetOrigin) {
              ...
              log2("Message targetOrigin: " + targetOrigin);
            }
          }
          function sendToParent() {
            var size = height2 + ":" + width2, message = myID + ":" + size + ":" + triggerEvent + (undefined2 === msg ? "" : ":" + msg);
            log2("Sending message to host page (" + message + ")");
        ...
        function receiver(event) {
          var processRequestFromParent = {
            ...
            reset: function resetFromParent() {
              if (initLock) {
                log2("Page reset ignored by init");
              } else {
                log2("Page size reset by host page");
            ...
            pageInfo: function pageInfoFromParent() {
              var msgBody = getData();
              log2("PageInfoFromParent called from parent: " + msgBody);
              onPageInfo(JSON.parse(msgBody));
              log2(" --");
            },
            message: function messageFromParent() {
              var msgBody = getData();
              log2("onMessage called from parent: " + msgBody);
  ...
  (async () => {
    try {
      const ctx = await import_bridge.view.getContext();
    ...
    try {
      const r = await (0, import_bridge.invoke)("ping", { from: "config-panel" });
  ...
  document.getElementById("save").addEventListener("click", () => {
    import_bridge.view.submit({ config: { spikeSavedAt: "M0-6" } }).catch((e) => log("submit ERROR: " + e.message));
  });
  document.getElementById("cancel").addEventListener("click", () => {
    import_bridge.view.close().catch((e) => log("close ERROR: " + e.message));
  });
})();
